...
allocateString();   // Write your allocateString() prototype here
...
char * allocateString(int numChars){
    ...
    for your solution you need to use sizeof(char) in this line, even if sizeof(char) returns 1. */

    int * ptr = (int * ) malloc( numChars * sizeof(char));
